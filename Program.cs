...
namespace MyApp
{
    internal class Program
    {
        static void Main(string[] args)  //entry point
        {
            ...
            //somma i primi 10000 numeri presi in modo Random

            Random rndnum = new Random();
            double[] listaNum = new double[10000];
            double iSommaValori = 0.0;

            for (int i = 0; i < listaNum.Length; i++)
            {
                listaNum[i] = rndnum.Next(0, 100);
                iSommaValori = iSommaValori + listaNum[i];
                Console.Write("{0} ", listaNum[i]);
            }

            double dMediaValori = iSommaValori / listaNum.Length;
            Console.WriteLine();
